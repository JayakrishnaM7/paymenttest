...
@Service
public class PaymentinitRestProcessor implements RestApiInterface {
	private static final Logger logger = LoggerFactory.getLogger(PaymentinitRestProcessor.class);

    @Autowired
	private Utility utility;

	@Value("${spring.api.paymentinit.hosturl}")
	private String url;
	
	@Value("${spring.api.paymentinit.context-path}")
	private String contextPath;

	@Value("${spring.api.paymentinit.jolt-filename}")
	...
	public Object exceptionMessage(Exception e, String requestObj) {//NOSONAR
		...
		if(e instanceof  HttpClientErrorException){
			if(((HttpClientErrorException) e).getStatusCode().equals(HttpStatus.NOT_FOUND) ||
				...
				apiError.setMessage("API called failed in " +  PaymentinitRestProcessor.class.getName());
			...
			error.setMessage("Error occurred in "+ PaymentinitRestProcessor.class.getName());
